...
- **Worker** — every `/api/watch-party/*` route returns 404 unless the `WATCH_PARTY_ENABLED` secret equals `"true"`. It is declared in `wrangler.jsonc` `secrets.required` (declaration only — a secret name listed there replaces `.dev.vars` inference, so the worker would never see the var otherwise) and set to `"true"` in `.dev.vars` for local dev/preview. In production it must exist (`wrangler deploy` **fails** if a required secret is unset) but is set to `"false"`, which keeps the API disabled. Toggle with `wrangler secret put WATCH_PARTY_ENABLED` (`true` to enable, `false` to disable). `WATCH_PARTY_SIGNING_SECRET` must likewise exist in production for deploys to pass, even while the feature is off.
